...
* TODO Recorder
** DONE idea
   Press record. Play stuff. Press stop.
   That gets stored to "latest loop".
   Then I can enter functions to do stuff with it --
   e.g. collect all pitches, map that to a key, etc.
...
*** extant types
  ...
  data Voice app = Voice {
      _voiceSynth  :: Maybe (Synth ZotParams)
  ...
  data EdoConfig = EdoConfig
    { _edo :: Int
** It has a start time.
** It can't keep a Vector (Ev label a) } like Museqs do
   because those assume you know the end time
** `data Observation label a`
   Unlike the Ev type, this is not abstract in the time type;
   it always uses Time, never RTime.
** It grows a list of Observations.
   from the head, so the neweset are first
** LATER
*** It has a set of event sources.
*** A monome button can be designated for "stop recording".
